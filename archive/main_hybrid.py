"""
main_hybrid.py - Training Script for Hybrid CNN + Power Spectrum Model
=======================================================================

Usage:
    python train/main_hybrid.py --run_name hybrid_v1
"""
...
from train.config import (
    ...
    SEED, GRADIENT_ACCUMULATION_STEPS, USE_AMP,
    USE_POWER_SPECTRUM, N_POWER_BINS,
    # Model
    CosmoNetHybrid, count_parameters,
    # Data
    create_dataloaders,
    # Device
    get_device,
...
# =============================================================================
# TRAINING FUNCTIONS (HYBRID)
# =============================================================================


def train_epoch_hybrid(model, loader, criterion, optimizer, device, scaler=None, grad_accum_steps=1):
    ...
    for batch_idx, batch in enumerate(loader):
        # Unpack batch (image, power_spectrum, target)
        fields, power_spectrum, targets = batch
        fields = fields.to(device, non_blocking=True)
        power_spectrum = power_spectrum.to(device, non_blocking=True)
        targets = targets.to(device, non_blocking=True)
        
        # Mixed precision forward pass
        if scaler is not None:
            with torch.amp.autocast('cuda'):
                outputs = model(fields, power_spectrum)
...
def validate_hybrid(model, loader, criterion, device, scaler=None):
...
def main(run_name: str):
    ...
    logging.info(f"Model: CosmoNetHybrid (CNN + Power Spectrum)")
    logging.info(f"Power spectrum bins: {N_POWER_BINS}")
    ...
    wandb.init(
        ...
        config={
            ...
            "patience": PATIENCE,
            "seed": SEED,
            "power_spectrum_bins": N_POWER_BINS,
    ...
    if not USE_POWER_SPECTRUM:
        logging.error("USE_POWER_SPECTRUM must be True for hybrid model!")
        return
    ...
    # Model
    model = CosmoNetHybrid().to(device)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        # Train
        train_loss = train_epoch_hybrid(model, train_loader, criterion, optimizer, device, 
                                        scaler=scaler, grad_accum_steps=GRADIENT_ACCUMULATION_STEPS)
        
        # Validate
        val_loss = validate_hybrid(model, val_loader, criterion, device, scaler=scaler)
    ...
    test_loss = validate_hybrid(model, test_loader, criterion, device, scaler=scaler)
    logging.info(f"Test loss: {test_loss:.6f}")
    logging.info(f"Test RMSE: {test_loss**0.5:.6f}")
    wandb.log({"test_loss": test_loss, "test_rmse": test_loss**0.5})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train CosmoNetHybrid")
